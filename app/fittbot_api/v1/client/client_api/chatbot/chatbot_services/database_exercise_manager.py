"""
Database-first exercise management system (now uses JSON from workouts.json)
Ensures all exercises come from the JSON catalog and provides AI-powered suggestions
"""
...
class DatabaseExerciseManager:
    """Manages exercise selection and validation using only database exercises"""

    @staticmethod
    def get_available_exercises_by_muscle(db: Session, muscle_group: str) -> List[Dict[str, Any]]:
        """Get all available exercises for a specific muscle group from database"""
    ...
    @staticmethod
    def validate_exercise_exists(db: Session, exercise_name: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    @staticmethod
    def find_similar_exercises(db: Session, exercise_name: str, limit: int = 3) -> List[Dict[str, Any]]:
        """Find similar exercises in the database when exact match fails"""
        catalog = load_catalog(db)
    ...
    @staticmethod
    def generate_muscle_group_template(db: Session, muscle_groups: List[str], exercises_per_group: int = 3) -> Dict[str, List[Dict[str, Any]]]:
    ...
    @staticmethod
    def validate_user_exercise_request(db: Session, user_request: str) -> Dict[str, Any]:
        """Validate and process user exercise requests using AI and database lookup"""
    ...
    @staticmethod
    def select_balanced_exercises(db: Session, muscle_groups: List[str], total_exercises: int = 6) -> List[Dict[str, Any]]:
