...
router = APIRouter(prefix="/api/admin/complimentary", tags=["AdminComplimentary"])


@router.get("/list")
async def get_complimentary_users(
    ...
    """
    Get list of clients with complimentary subscriptions
    Complimentary users are identified by:
    ...
    try:
        ...
        # Get all unique customer_ids with complimentary subscriptions
        ...
        # Debug: Print all customer_ids found
        print(f"[COMPLIMENTARY] Customer IDs with internal_manual: {customer_ids}")

        # Debug: Check if any customer has multiple subscriptions
        for cid in customer_ids:
            all_subs_stmt = select(
                Subscription.provider,
                Subscription.status,
                Subscription.active_from,
                Subscription.active_until
            ).where(Subscription.customer_id == cid)
            all_subs_result = await db.execute(all_subs_stmt)
            all_subs = all_subs_result.all()
            print(f"[COMPLIMENTARY] Customer {cid} has {len(all_subs)} subscriptions:")
            for sub in all_subs:
                print(f"  - provider={sub.provider}, status={sub.status}, active_from={sub.active_from}, active_until={sub.active_until}")

        if not customer_ids:
            # No complimentary users found
            return {
                "success": True,
                "data": {
                    "users": [],
                ...
                "message": "Complimentary users fetched successfully"
            }
        ...
        most_recent_subq = (
            ...
            .where(Subscription.provider == 'internal_manual')
        ...
        # The count will show total complimentary users, regardless of search

        print(f"[COMPLIMENTARY] Count: {total_count}")
        print(f"[COMPLIMENTARY] Users fetched: {len(users)}")
        ...
        for row in users:
            ...
            complimentary_users.append(user_data)
        ...
        return {
            "success": False,
            "message": f"Error fetching complimentary users: {str(e)}"
        }


@router.get("/stats")
async def get_complimentary_stats(
    db: AsyncSession = Depends(get_async_db)
):
    """
    Get overall complimentary statistics
    """
    try:
        today = datetime.now().date()

        # Total active complimentary subscriptions
        total_stmt = select(func.count(distinct(Subscription.customer_id))).where(
            Subscription.provider == 'internal_manual',
            Subscription.status == 'active'
        )
        total_result = await db.execute(total_stmt)
        total_complimentary = total_result.scalar() or 0

        # Complimentary expiring in next 7 days
        ...
        # New complimentary subscriptions started in last 7 days
        ...
        return {
            "success": True,
            "data": {
                "total": total_complimentary,
                "expiring_soon": expiring_soon,
                "new_this_week": new_trials
            },
            "message": "Complimentary stats fetched successfully"
        }
        ...
        return {
            "success": False,
            "message": f"Error fetching complimentary stats: {str(e)}"
        }
